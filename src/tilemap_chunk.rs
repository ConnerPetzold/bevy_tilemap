...
use super::{
    ATTRIBUTE_TILE_INDEX, TileData, TileStorage, TilemapChunkMaterial, TilemapInfo, TilemapLayer,
    TilemapRenderMode, Tileset,
};
    ...
    for (tilemap_layer_entity, tilemap_layer, mut tile_storage, tileset) in tilemap_layer_query {
        ...
        if let Some(tileset_image) = images.get(&tileset.image) {
            ...
            if layer_count % 6 == 0 {
                ...
                if layer_count == 6 {
                    error!(
                        "WebGL2: Tileset image has 6 layers which WebGL2 will interpret as a Cube texture. Ensure the layer count is not a multiple of 6."
                    );
                } else {
                    error!(
                        "WebGL2: Tileset image has {} layers. This is a multiple of 6, which WebGL2 will interpret as a CubeArray texture. Ensure the layer count is not a multiple of 6.",
                        layer_count
                    );
                }
            }
